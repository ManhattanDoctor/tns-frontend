...
export class AuctionResolver implements Resolve<Auction> {
    ...
    public async resolve(route: ActivatedRouteSnapshot, state: RouterStateSnapshot): Promise<Auction> {
        let id = route.params.id;
        if (_.isNil(id)) {
            let message = 'error.auctionIdNotFound';
        ...
        try {
            return await this.api.auctionGet(id);
        } catch (error) {
            this.router.navigate(RouterService.DEFAULT_URL);
